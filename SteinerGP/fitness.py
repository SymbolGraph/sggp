...
The :mod:`steiner.fitness` module contains some metric with which to evaluate
the computer programs created by the :mod:`steiner.genetic` module.
"""
...
def r2_score(y, y_pred, w):
    """Calculate the r2 score."""
    y_mean= np.average(y, weights=w)
    y_=np.sum(((y - y_mean) ** 2))
    return np.sum(((y - y_pred) ** 2))/y_
